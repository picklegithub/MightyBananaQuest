...
interface Props {
  ...
  disabled?: boolean
  done?: boolean  // right-swipe shows amber undo reveal instead of green complete
}

const THRESHOLD  = 64   // px to trigger action
const MAX_TRAVEL = 96   // px max drag distance shown

export function SwipeableRow({ onComplete, onDelete, children, disabled, done }: Props) {
  ...
  return (
    ...
    >
      {/* ── Right reveal — green complete or amber undo ── */}
      {onComplete && (
        <div style={{
          ...
          {done
            ? <Icons.back size={20} style={{ color: 'white' }} />
