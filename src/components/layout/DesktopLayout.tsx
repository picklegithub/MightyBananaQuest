...
// ── Types ─────────────────────────────────────────────────────────────────────
type DesktopTab = 'dashboard' | 'journal' | 'goals' | 'calendar' | 'inbox' | 'all-tasks' | 'all-habits' | 'review' | 'settings'
...
function DesktopRow({
  ...
  const hue = isColorful ? cat?.hue : undefined
  const accentColor  = hue !== undefined ? `hsl(${hue}, 45%, 55%)` : 'transparent'
  const completeColor = hue !== undefined ? `hsl(${hue}, 45%, 55%)` : 'var(--accent)'
  return (
    <div
      onClick={onSelect}
      style={{
        display: 'flex', alignItems: 'center', gap: 12,
        padding: '11px 14px 11px 11px', borderRadius: 8, cursor: 'pointer', marginTop: 2,
    ...
    >
      <button
        ...
        style={{
          ...
          border: `1.5px solid ${task.done ? completeColor : 'var(--ink-3)'}`,
          background: task.done ? completeColor : 'transparent',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}
      >
      ...
      <I size={13} stroke={hue !== undefined ? accentColor : 'var(--ink-3)'} />
...
export function DesktopLayout({ onLogout }: Props) {
  ...
  const desktopNavigate = useCallback((s: Screen) => {
    ...
    else if (s.name === 'all-habits') setTab('all-habits')
  }, [])
  ...
  // ── FAB handlers — mirrors mobile App.tsx ─────────────────────────────────
  function handleFabTap()       { setFabSheet('capture') }
  ...
  const todayTasks    = filteredTasks.filter(t => t.due === 'Today' && !t.isHabit)
  const tomorrowTasks = filteredTasks.filter(t => t.due === 'Tomorrow' && !t.isHabit)
  const weekTasks     = filteredTasks.filter(t => !['Today','Tomorrow'].includes(t.due) && !t.isHabit)
  ...
  function handleTabChange(s: Screen) {
    ...
    const desktopTabMap: Record<string, DesktopTab> = {
      ...
      'all-habits': 'all-habits', review: 'review', settings: 'settings',
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--paper)', color: 'var(--ink)', fontFamily: 'var(--font-ui)', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ width: 220, borderRight: '1px solid var(--rule)', display: 'flex', flexDirection: 'column', background: 'var(--paper-2)', flexShrink: 0 }}>
          ...
          <div style={{ padding: '0 10px', flex: 1, overflowY: 'auto' }} className="no-scrollbar">
            ...
            {([
              ...
              { id: 'all-habits', label: 'Habits',    icon: 'repeat'   },
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
            ...
            {tab === 'goals' && (
              <div style={{ flex: 1, overflowY: 'auto' }} className="no-scrollbar">
                <GoalsScreen navigate={desktopNavigate} back={desktopBack} onAddTask={() => openAddTask()} />
              </div>
            )}
            ...
            {tab === 'inbox' && (
              <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
                <div style={{ flex: 1, overflowY: 'auto' }} className="no-scrollbar">
                  <InboxScreen navigate={desktopNavigate} back={desktopBack} />
                </div>
                {selectedTask && (
            ...
            {tab === 'all-habits' && (
              <div style={{ flex: 1, overflowY: 'auto' }} className="no-scrollbar">
                <AllHabitsScreen navigate={desktopNavigate} back={desktopBack} />
              </div>
            )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6, marginBottom: 6 }}>
                  {catFilter && (
                    <button
                      ...
                      style={{ color: 'var(--ink-2)', display: 'flex', alignItems: 'center', flexShrink: 0 }}
                    >
                      <Icons.home size={22} />
                    </button>
                  )}
                  <div className="t-display" style={{ fontSize: 22, lineHeight: 1.05 }}>
      ...
      {/* ── Sheets / modals — same set as mobile ── */}
      {fabSheet === 'menu' && (
      ...
      {fabSheet === 'capture' && (
        <QuickCaptureSheet
          onClose={closeSheet}
          onExpand={title => { setTaskPrefill({ title }); setFabSheet('task') }}
        />
      )}
      ...
      {fabSheet === 'goal' && cats.length > 0 && (
        <AddGoalSheet categories={cats} onClose={closeSheet} />
      )}
