...
import { isDueToday } from './parseDue'
...
export function useNotifications(
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      const openTasks = await db.tasks.filter(t => !t.done && !t.isHabit).toArray()

      const overdue  = openTasks.filter(t => isoRe.test(t.due) && t.due < todayISO)
      const dueToday = openTasks.filter(t => isDueToday(t.due))
  ...
  useEffect(() => {
    ...
    async function scheduleDueTimeNotifs() {
      ...
      // Grab all incomplete tasks due today with a specific time set
      const tasks = await db.tasks
        .filter(t => !t.done && !t.isHabit && !!t.time && isDueToday(t.due))
        .toArray()
