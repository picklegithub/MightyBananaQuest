...
/** True if `due` represents today — handles both 'Today' string and ISO date */
export function isDueToday(due: string): boolean {
  if (!due) return false
  if (due === 'Today') return true
  try { return isToday(new Date(due + 'T00:00:00')) } catch { return false }
}

/** True if `due` represents tomorrow — handles both 'Tomorrow' string and ISO date */
export function isDueTomorrow(due: string): boolean {
  if (!due) return false
  if (due === 'Tomorrow') return true
  try { return isTomorrow(new Date(due + 'T00:00:00')) } catch { return false }
}
...
export function formatDueLabel(due: string): string {
  if (!due) return 'Today'
  // Legacy relative strings (no absolute date stored — display as-is)
...
/**
 * Converts a next-occurrence ISO date to a human-friendly "Next" label.
 * Used by the inline recurring-task completion banner.
 *   today      → "Today"
 *   tomorrow   → "Tomorrow"
 *   within 7d  → "Thursday"
 *   otherwise  → "12 Jun"
 */
export function nextDueLabel(isoDate: string): string {
  ...
  if (diffDays <= 7)  return next.toLocaleDateString('en-AU', { weekday: 'long' })
